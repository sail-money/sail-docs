...
You are responsible for the correctness of any permission you register (see [permission correctness is the author's responsibility](../security/limitations.md)). Treat these templates as starting points to read, learn from, and adapt — not audited, drop-in production contracts. The seven shared templates are deployed against the current kernel (see [addresses](../reference/addresses.md)). Six were included in the [Octane security review](../security/audits.md); the seventh, `WithdrawPermission`, was rewritten afterward (v2) and is not covered by it. All remain reference implementations; verify and test before any production use.
{% endhint %}
